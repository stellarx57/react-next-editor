...
var image = {
  ...
  parseDOM: [
    {
      ...
      getAttrs(dom) {
        const el = dom;
        const src = chunkSRDIWQEX_cjs.sanitizeImageSrc(el.getAttribute("src"));
        if (!src) return false;
  ...
  toDOM(node) {
    ...
    const safeSrc = chunkSRDIWQEX_cjs.sanitizeImageSrc(src) ?? "";
...
var link = {
  ...
  parseDOM: [
    {
      ...
      getAttrs(dom) {
        ...
        const href = chunkSRDIWQEX_cjs.sanitizeUrl(rawHref);
  ...
  toDOM(mark) {
    const href = chunkSRDIWQEX_cjs.sanitizeUrl(mark.attrs.href) ?? "";
...
function buildSchema(features = {}) {
  const f = { ...chunkAT25KOMU_cjs.DEFAULT_FEATURES, ...features };
  const nodes = {
    doc,
    paragraph
  };
...
function setLink(type, attrs) {
  return (state, dispatch) => {
    const href = chunkSRDIWQEX_cjs.sanitizeUrl(attrs.href);
    if (!href) return false;
...
function insertImage(schema, attrs) {
  return (state, dispatch) => {
    ...
    if (!type) return false;
    const src = chunkSRDIWQEX_cjs.sanitizeImageSrc(attrs.src);
    if (!src) return false;
    if (dispatch) {
  ...
  return new prosemirrorState.Plugin({
    props: {
      transformPastedHTML(html) {
        return chunkSRDIWQEX_cjs.sanitizeHtmlSync(html);
      }
    }
  });
}
...
//# sourceMappingURL=chunk-QSA3ZJBC.cjs.map
//# sourceMappingURL=chunk-QSA3ZJBC.cjs.map